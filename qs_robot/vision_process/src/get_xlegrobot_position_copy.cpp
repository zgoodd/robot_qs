    ...
    for (i = 0; i < height; i++)
        for (j = 0; j < width; j++)
        {
            ...
            */
            ...
            else if (ChooseObject == 1)  //银色金属管
            {
                if (!( (s_hsv.val[0] > 11) && (s_hsv.val[0] < 34) && (s_hsv.val[1] > 150) ))
                {
                    s.val[0] = 0;  //其他以外的都白色（255,255,255）或黑色（0,0,0）
                    s.val[1] = 0;
                    s.val[2] = 255;
                    cvSet2D(hsv, i, j, s);    //把新图像s的值，赋给hsv指针所指向的图像
                }
...
//             CvScalar sval2 = cvGet2D(hsv, 178, 338);//获取像素点为（338, 178）点的HSV的值
//             qDebug("sval.val[0,1,2] = %f, %f, %f", sval2.val[0], sval2.val[1], sval2.val[2]);
